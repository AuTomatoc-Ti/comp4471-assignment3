...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += -torch.log(numerator_k / denominator_k)
        total_loss += -torch.log(numerator_k / denominator_k_N)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = torch.cat([torch.diag(sim_matrix, diagonal=N), torch.diag(sim_matrix, diagonal=-N)], dim=0).unsqueeze(1)
