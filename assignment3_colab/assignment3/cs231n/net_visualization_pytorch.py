...
def compute_saliency_maps(X, y, model):
    ...
    saliency = X.grad.detach().abs().max(dim=1)[0]
...
def make_fooling_image(X, target_y, model):
    ...
    for _ in range(100):
        ...
        if grad_norm > 0:
            X_fooling = X_fooling + learning_rate * grad / grad_norm
        X_fooling = X_fooling.detach().requires_grad_()
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    model.zero_grad()
    scores = model(img)
    objective = scores[0, target_y] - l2_reg * torch.sum(img * img)
    objective.backward()
    with torch.no_grad():
